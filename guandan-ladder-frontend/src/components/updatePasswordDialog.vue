...
// 处理保存输入框值的逻辑 调用修改密码接口 并 判断原密码
  if (formModel.oldPassword == "" || formModel.newPassword == "" || inputValue3.value == "") {
    alert("空的不行哦")
  } else if (inputValue3.value != formModel.newPassword) {
    alert("新密码和确认密码不一致哦")
  } else {
  ...
  <v-dialog v-model="updatePasswordDialog" persistent>
    <v-card>
      ...
      <v-card-text>
        ...
        <v-text-field v-model="inputValue3" label="确认密码" required></v-text-field>
      </v-card-text>
      <v-card-actions>
